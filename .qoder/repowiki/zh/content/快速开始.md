...
G[project.private.config.json] --> H[私有配置]
I[sitemap.json] --> J[搜索引擎配置]
subgraph "页面目录 pages/"
K[index/] --> L[首页图表]
M(statistics/) --> N[明细统计]
O(addRecord/) --> P[记账页面]
...
**新增** 私有配置文件包含本地开发特定的设置，提供了更高级的开发功能和调试优化选项：
...
1. **高级开发设置**
   - `skylineRenderEnable`: 启用Skyline渲染引擎
   - `useApiHook`: 启用API钩子功能
   - `showShadowRootInWxmlPanel`: 在WXML面板显示Shadow Root

2. **库版本管理**
   - `libVersion`: 指定小程序基础库版本（3.15.1）
   - 支持不同版本的基础库配置

3. **调试优化功能**
   - `compileHotReLoad`: 热重载功能
   - `ignoreDevUnusedFiles`: 忽略未使用文件
   - `checkInvalidKey`: 无效键检查

4. **性能优化选项**
   - `preloadBackgroundData`: 预加载后台数据
   - `bigPackageSizeSupport`: 大包大小支持
...
**图表来源**
- [pages/statistics/statistics.js:19-200](file://pages/statistics/statistics.js#L19-L200)
...
**新增** 项目现在支持双层配置系统，提供了更精细的开发控制：

1. **基础配置**：通过 `project.config.json` 控制通用开发设置
2. **私有配置**：通过 `project.private.config.json` 控制高级开发功能
3. **版本管理**：独立的基础库版本控制
4. **调试优化**：专门的调试和性能优化选项
